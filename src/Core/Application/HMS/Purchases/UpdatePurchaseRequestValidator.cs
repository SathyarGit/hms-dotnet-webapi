...
public class UpdatePurchaseRequestValidator : CustomValidator<UpdatePurchaseRequest>
{
    public UpdatePurchaseRequestValidator(IReadRepository<Purchase> purchaseRepo, IReadRepository<Department> departmentRepo, IReadRepository<Vendor> vendorRepo, IReadRepository<Transactionstatus> transactionstatusRepo, IStringLocalizer<UpdatePurchaseRequestValidator> T)
    {
        RuleFor(p => p.Image)
            .InjectValidator();
